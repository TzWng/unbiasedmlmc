ggplot_box_legend <- function(family = "serif"){
  ...
  sample_df$values[1:100] <- 601:700
  ...
  ggplot2_boxplot <- function(x){
    ...
    upper_whisker <- as.numeric(quantile(x, 0.9))
    lower_whisker <- as.numeric(quantile(x, 0.05))
  ...
  explain_plot <- ggplot() +     
    ...
    geom_boxplot(data = sample_df, aes(x = parameter, y=values), width = 0.3, fill = "white", fatten=1) + 
    stat_summary(geom = "boxplot", fun.data = function(x) setNames(quantile(x, c(0.1, 0.25, 0.5, 0.75, 0.85)), 
    ...
    theme_minimal(base_size = 3, base_family = family) +     
    ...
    geom_text(aes(x = 2.4, y = ggplot_output[["mean"]]), label = "Interquartile\nrange",vjust = 0.4) +     
    geom_text(aes(x = c(1.17,1.17),  y = c(ggplot_output[["upper_whisker"]], ggplot_output[["lower_whisker"]]), label = c("97.5th percentile",  "2.5th percentile"))) +     
    ...
    coord_cartesian(xlim = c(1.4,3.1), ylim = c(-80, 650)) +
