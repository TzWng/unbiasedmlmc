...
#'}
#'@export
sample_unbiasedestimator <- function(single_kernel, coupled_kernel, rinit, h, k = 0, m = 1, lag = 1, max_iterations = Inf){
  if (k > m){
  ...
  return(list(mcmcestimator = mcmcestimator / (m - k + 1), correction = correction / (m - k + 1), uestimator = uestimator / (m - k + 1),
              meetingtime = meetingtime, iteration = time, cost = cost))
}


# multiple Unbiased MCMC estimators
#'@export
get_unbiasedmcmc <- function(single_kernel, coupled_kernel, rinit, h, k = 500, m = 501, lag = 1, max_iterations = Inf, nrep){
  Samples = sample_unbiasedestimator(single_kernel, coupled_kernel, rinit,
                                     h, k, m, lag = 1, max_iterations = Inf)
  if(is.null(dim(Samples$uestimator))){d = 1
  } else {d = ncol(Samples$uestimator) }
  Samples = matrix(nrow = nrep, ncol = d)
  for ( i in 1:nrep){
    temp = sample_unbiasedestimator(single_kernel, coupled_kernel, rinit,
                                    h, k, m, lag = 1, max_iterations = Inf)
    Samples[i,] = temp$uestimator
  }
  return(Samples)
}
