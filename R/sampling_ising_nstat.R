...
#' unbiased estimators for the expected normalizing constant
...
#'@export
sampling_ising_nstat <- function(betas, k = 0, m = 1, nrep, size){
  # number of dimension
  d <- length(betas)
  uestimators <- matrix(NA, ncol = d, nrow = nrep)
  for(i in 1:d){
    for(j in 1:nrep){
      result <- get_ising_nstat(betas[i], k = k, m = m, size)
